<script lang="ts">
    import { plotsNotInCollections, savedCollections } from "../data";
    ...
    function openSharingWithColl() {
        console.log("open sharing with coll", item.name);
        return sharing(key)
    }
</script>


<div class="card folder bg-base-200 hover:bg-accent/15 hover:-translate-y-1 transition-all duration-150 max-w-sm aspect-[4/3] shadow-lg">
    
    <div class="card-body relative">
        <a class="absolute w-full h-full hover:bg-secondary/15 transition-colors duration-150 rounded-xl inset-0" 
            href="#/dash/{link}" title="Open Collection"> </a>
        <span class="card-title mt-6 z-[5] text-primary">{item.name}</span>
        ...
        <div class="mt-auto card-actions items-center">
            ...
            <button title={item.public ? "Plot is public": "Plot is private" } type="button" on:click={openSharingWithColl}
...
<style type="postcss">
    ...
    .folder {
        ...
        mask-image: url('../icons/folder-filled-4x3.svg');
