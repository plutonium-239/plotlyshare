...
export declare namespace BaseProvider {
    ...
    interface TokensFromCodeOptions {
        clientId: string;
        clientSecret: string;
        redirectUrl?: string;
...
declare type RedirectFunction<T> = (options: BaseProvider.RedirectOptions) => Promise<T>;
declare type UsersFunction<T> = (options: BaseProvider.CallbackOptions) => Promise<T>;
export interface SocialProvider<T = any> {
    getUser(token: string, userAgent?: string): Promise<any>;
    getTokensFromCode(code: string, options: BaseProvider.TokensFromCodeOptions): Promise<OAuthTokens>;
